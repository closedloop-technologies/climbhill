...
import { attachAttemptPatch, compareAttempts, decideAttempt, evaluateAttempt, planImprovement, promoteSkill, recordReflection } from "../src/improvement.js";
...
test("recursive runs preserve lineage, evaluations, decisions, and skill provenance", async () => {
  ...
  const first = await planImprovement(root, initialized.controlWorktree, repo, { goal: "Improve evaluation", attempts: 1 });
  const child = await planImprovement(root, initialized.controlWorktree, repo, { goal: "Refine evaluation", attempts: 1, parentRun: first.id, parentAttempt: first.attempts[0] });
  assert.equal(child.parentRun, first.id);
  assert.equal(child.parentAttempt, first.attempts[0]);
  const evaluation = await evaluateAttempt(root, repo, child.id, child.attempts[0], "node -e \"process.exit(0)\"", 10);
  assert.equal(evaluation.status, "passed");
  assert.equal((await compareAttempts(root, child.id))[0].passed, 1);
  const patch = join(parent, "attempt.patch");
  await writeFile(patch, "diff --git a/README.md b/README.md\n");
  assert.match(await attachAttemptPatch(root, child.id, child.attempts[0], patch, "deadbeef"), /\.patch$/);
  ...
  await assert.rejects(() => decideAttempt(root, child.id, child.attempts[0], "promote", "Passes", false), /--approve/);
  await decideAttempt(root, child.id, child.attempts[0], "promote", "All required checks pass.", true);
